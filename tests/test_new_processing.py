...
appia_dir = os.path.split(os.path.dirname(os.path.realpath(__file__)))[0]
...
class TestProcessing(unittest.TestCase):
    def test_waters_processing(self):
        waters_file = os.path.join(appia_dir, "test-files", "results1844.arw")
    ...
    def test_old_shim_processing(self):
        shim_file = os.path.join(appia_dir, "test-files", "05_25_BB.asc")

        results = hplc.OldShimProcessor(
            shim_file, hplc_flow_rate=0.5, channel_mapping={"A": "Trp", "B": "GFP"}
        )
    ...
    def test_new_shim(self):
        shim_file = os.path.join(appia_dir, "test-files", "new-shim-1.txt")
        results = hplc.NewShimProcessor(shim_file, hplc_flow_rate=0.5)
    ...
    def test_agilent(self):
        ...
        results = hplc.AgilentProcessor(
            ag_file, hplc_flow_rate=0.5, agilent_channel_name="test-channel"
        )
    ...
    def test_akta(self):
        ...
        results = fplc.AktaProcessor(akta_file, fplc_cv=24)
        df = results.df
