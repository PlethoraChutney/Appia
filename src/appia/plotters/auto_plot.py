    ...
    for i in [0, 1]:
        if input_limits[i] == "auto":
            ...
            try:
                ...
                logging.warning(
                    f'Could not convert "{limits[i]}" to number. Using auto limit.'
                )
                limits.append(df_limits[i])
...
def auto_plot_fplc(df, limits, fractions, xax_var):
    ...
    try:
        ...
        logging.warning(
            f'Could not convert "{fractions[2]}" to integer. Plotting all fractions in range.'
        )
        fractions = range(frac_lims[0], frac_lims[1] + 1)
    ...
    fig.update_yaxes(matches=None).update_xaxes(range=limits)
