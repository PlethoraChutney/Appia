...
def main(args):
    ...
    if args.flow_rate is not None:
        ...
        for fr_pair in args.flow_rate:
            try:
                ...
                logging.error(
                    "Bad flow rate. Give as --flow-rate {name} {mL/min number}"
                )
                raise TypeError
    ...
    if args.database_setup:
        print(
            "Setting up database. To leave any of these settings unchanged, leave them blank."
        )

        new_settings = {}
        new_settings["database_host"] = input(
            "Database host (something like blah.domain.edu): "
        )
        new_settings["database_user"] = input(
            "Database username. (You set this during the docker installation): "
        )
        new_settings["database_password"] = input(
            "Database password. (You set this during the docker installation): "
        )
        port = input(
            "Database port number. (If you don't know, it's the default so leave it blank): "
        )
...
parser.add_argument(
    "--set-fplc-cv", help="Set default CV for all FPLC traces.", type=float
)
parser.add_argument(
    "--remove-fplc-cv",
...
parser.add_argument(
    "--database-setup", help="Set database access parameters", action="store_true"
)
parser.add_argument(
