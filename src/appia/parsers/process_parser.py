...
def main(args):
    ...
    for i, filename in enumerate(file_list):
        ...
        if len(claimed) == 1:
            ...
            logging.warning(
                f"{filename} claimed by no processor. If it is not a chromatography trace, this is fine."
            )
    ...
    if args.output_dir:
        ...
        out_dir = os.path.abspath(
            os.path.join(os.curdir, exp.id.replace(os.path.sep, "-"))
        )
    ...
    if not args.no_move:
        for claimed_file in processed_files:
            raw_data_dir = os.path.join(
                out_dir, f"{claimed_file.manufacturer}_raw-files"
            )
            if not os.path.isdir(raw_data_dir):
                os.makedirs(raw_data_dir)

            new_file_path = os.path.join(
                raw_data_dir, os.path.basename(claimed_file.filename)
            )
    ...
    if args.plots:
        if exp.hplc is not None:
            ...
            auto_plot.auto_plot_hplc(exp.hplc, args.ml, "mL").write_image(
...
parser = argparse.ArgumentParser(
    description="Process chromatography data", add_help=False
)
...
auto_plots.add_argument(
    "-p", "--plots", help="Make default plots", action="store_true", default=False
)
auto_plots.add_argument(
    "-f",
    "--fractions",
