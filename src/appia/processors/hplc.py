...
class HplcProcessor(object):
    ...
    def flow_rate(self) -> float:
        logging.debug(
            f"Flow rate info: self: {self._flow_rate}, override: {self.__class__.flow_rate_override}"
        )
        if self._flow_rate is not None:
            ...
            # otherwise prompt the user
                ...
                while not isinstance(self._flow_rate, float):
                    try:
                        self.flow_rate = float(
                            input(
                                f"Please enter a flow rate for {self.sample_name} (mL/min): "
                            )
                        )
                    except ValueError:
                ...
                if (
                    input(
                        f"Set all remaining {self.manufacturer} trace flow rates to {self._flow_rate} for this experiment? (y/n)"
                    ).lower()
                    == "y"
                ):
                ...
                if self.method is not None:
                    if (
                        input(
                            f"Save flow rate of {self._flow_rate} for all future traces using method {self.method}? (y/n)"
                        ).lower()
                        == "y"
                    ):
                        ...
                        print(
                            f"Saved flow rate: {self.method} = {self._flow_rate}.\nYou can change it later using appia utils."
                        )
    ...
    def df(self) -> pd.DataFrame:
        ...
        return self._df[["Time", "mL", "Channel", "Sample", "Normalization", "Value"]]
    ...
    def set_name(self, new_set_name):
        self._set_name = (
            None if new_set_name is None else new_set_name.replace(os.path.sep, "-")
        )


class WatersProcessor(HplcProcessor):
    def __init__(self, filename: str, **kwargs):
        super().__init__(filename, manufacturer="Waters", **kwargs)
    ...
    def prepare_sample(self) -> None:
        sample_info = pd.read_csv(
            self.filename, delim_whitespace=True, nrows=2, dtype=str
        )
...
class OldShimProcessor(HplcProcessor):
    def __init__(self, filename, **kwargs):
        ...
        super().__init__(filename, manufacturer="Shimadzu", **kwargs)
    ...
    def process_file(self) -> None:
        ...
        for i in range(len(self.data_points)):
            time_column.extend(
                [x * self.sampling_rate[i] for x in range(self.data_points[i])]
            )
...
class NewShimProcessor(HplcProcessor):
    ...
    def prepare_sample(self) -> None:
        ...
        for key in tables:
            if re.match("LC Chromatogram", key):
                chrom_channel = re.search(r"Chromatogram\((.*?)\)", key)
        ...
        if len(set(detector_channel_pairs.values())) != len(
            detector_channel_pairs.values()
        ):
            counter = {}
            ...
            for dup in duplicate_channels:
                ...
                if NewShimProcessor.prefer_detector is not None:
                    ...
                    is_preferred = [
                        x
                        for x in duped_detectors
                        if NewShimProcessor.prefer_detector + "-Ch" in x.split(" ")[1]
                    ]
                    if len(is_preferred) == 1:
                        selected_detector = is_preferred[0]
                    elif len(is_preferred) > 1:
                        logging.warning(
                            f'Preferred detector matched more than once: {", ".join(is_preferred)}'
                        )
                    else:
                        logging.info(
                            f"Preferred detector {NewShimProcessor.prefer_detector} does not match any duplicated channel."
                        )

                if selected_detector is None:
                    logging.warning(
                        "Duplicate channels detected. You can use --prefer-detector to select one in the future."
                    )
                    print(
                        f"Select a detector for {dup}. Any non-numeric choice will default to 0."
                    )
                    ...
                    try:
                        ...
                        NewShimProcessor.prefer_detector = re.search(
                            r"Detector (.*?)-Ch[0-9]*", selected_detector
                        ).group(1)
    ...
    def process_file(self) -> None:
        ...
        for detector, chrom in self.chroms.items():
            ...
            info_lines = chrom[:15]
            info = {}
            info_patterns = {"interval": "Interval(msec)", "num_samples": "# of Points"}
...
class AgilentProcessor(HplcProcessor):
    ...
    def __init__(self, filename, **kwargs):
        ...
        flow_match = re.search(AgilentProcessor.flow_pattern, filename)
        if flow_match:
            if flow_rate is not None:
                logging.warning(
                    "Agilent filename has a flow rate, but a flow rate was also passed as an argument. Using the argument flow rate. To use the filename flowrate, do not use --hplc-flow-rate option."
                )
            else:
                mod_args["hplc_flow_rate"] = float(flow_match.group(1))
        super().__init__(filename, manufacturer="Agilent", **mod_args)
    ...
    def channel(self) -> str:
        ...
        if channel_match:
            ...
            print(
                "To avoid entering manually in the future, include the channel in the filename like so:"
            )
            print("{filename}_Channel{channel name}_{rest of filename}.csv")

            if (
                input(
                    f"Set channel for all following Agilent files in this experiment to {self._channel}? (y/n) "
                ).lower()
                == "y"
            ):
    ...
    def prepare_sample(self):
        ...
        self.sample_name = re.sub(AgilentProcessor.flow_pattern, "_", self.sample_name)
