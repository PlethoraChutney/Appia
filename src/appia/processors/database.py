...
class Database:
    def __init__(self) -> None:
        ...
        if any([x is None for x in [username, password, hostname, port]]):
            ...
            try:
                ...
                logging.info(
                    "Appia no longer uses environment variables for couchDB access. Your current environment variables have been saved to the new user settings system and are no longer required."
                )
                logging.info(
                    "To update these values in the future, run `appia utils --database-setup`"
                )
                ...
                try:
                    logging.info(
                        "Appia no longer uses separate JSON files for couchDB login. If you use an Appia config file, please enter the location below to transfer the information to the new user settings system. Otherwise, leave the entry blank."
                    )
                    old_settings = input("Location of config.json: ")
                    old_settings = os.path.expanduser(old_settings)
                    if len(old_settings) == 0:
                        logging.error(
                            "Please run `appia utils --database-setup` to input database access information manually"
                        )
                        sys.exit(10)
    ...
    def pull_experiment(self, id):
        ...
        try:
            ...
            if doc["version"] == self.version:
                ...
                logging.info(
                    f"{id} is a v3 Experiment. You should re-upload this Experiment."
                )
            ...
            logging.error(
                "No version number. Check experiment ID and perform db migration."
            )
    ...
    def migrate(self):
        if (
            input(
                f"To migrate database hosted at {appia_settings.database_host}, type: I have backed up my db\n"
            ).lower()
