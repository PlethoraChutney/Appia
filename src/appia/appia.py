...
subparsers.add_parser(name="process", help="Process data", parents=[process_parser])
subparsers.add_parser(name="database", help="Manage CouchDB", parents=[db_parser])
subparsers.add_parser(name="utils", help="Utilities", parents=[util_parser])


def main():
    args = main_parser.parse_args()

    levels = {"q": logging.ERROR, "v": logging.INFO, "d": logging.DEBUG}
    try:
    ...
    logging.basicConfig(
        level=level, format="{levelname}: {message} ({filename})", style="{"
    )
